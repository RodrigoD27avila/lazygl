...
void LTexture::render(GLfloat x, GLfloat y, LFRect *clip)
{
	...
	// enable vertex and texture coordinate array
	...
	glTexCoordPointer(2, GL_FLOAT, sizeof(LVertexData2D),
		 (GLvoid *)offsetof(LVertexData2D, texCoord));
	
	// set vertex data
	glVertexPointer(2, GL_FLOAT, sizeof(LVertexData2D),
		 (GLvoid *)offsetof(LVertexData2D, position));
	
	// disable vertex and texture coordinate array
	glDisableClientState(GL_TEXTURE_COORD_ARRAY);
	glDisableClientState(GL_VERTEX_ARRAY);

	// draw quad vertex data and index data
...
{
	// if texture is not loaded and and vbo exists
	if (m_textureid == 0 && m_vboid != 0) {
		return;
	}
	...
	glBindBuffer(GL_ARRAY_BUFFER, m_vboid);
	glBufferData(GL_ARRAY_BUFFER, 4 * sizeof(LVertexData2D),
		vdata, GL_DYNAMIC_DRAW);

	// create IBO
	glGenBuffers(1, &m_iboid);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_iboid);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, 4 * sizeof(LVertexData2D),
